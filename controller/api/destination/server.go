...
func (s *server) GetProfile(dest *pb.GetDestination, stream pb.Destination_GetProfileServer) error {
	...
	// The service adaptor merges profile updates with service updates and
	// publishes the result to the traffic split adaptor.
	serviceAdaptor := newServiceAdaptor(tsAdaptor)

	// Subscribe the adaptor to service updates.
	err = s.services.Subscribe(service, serviceAdaptor)
	if err != nil {
		log.Warnf("Failed to subscribe to service updates for %s: %s", service, err)
		return err
	}
	defer s.services.Unsubscribe(service, serviceAdaptor)
	...
	primary, secondary := newFallbackProfileListener(serviceAdaptor)
